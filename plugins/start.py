...
async def Files_Option(bot:Client, message:Message):
    ...
    try:
        ...
        buttons = [[InlineKeyboardButton("Rename 📝", callback_data=f"rename-{message.from_user.id}")],
                   [InlineKeyboardButton("Compress 🗜️", callback_data=f"compress-{message.from_user.id}")]]
...
async def cancel_process(bot:Client, message:Message):
    ...
    try:
        ...
        return await message.reply_text(text="**cancelled all ongoing process ✅**")
